...
float last_cafeteria_entrance = -1;
float first_cafeteria_exit = -1;
float first_cafeteria_exit_without_food = -1;


//boolean fishs_first_exit = true;
//boolean fishs_first_entrance = true;
...
{   
    ...
    last_cafeteria_entrance = -1;
    first_cafeteria_exit = -1;
    first_cafeteria_exit_without_food = -1;
    ...
    while (fish_in_feeding_area == true) {
      //feeder_check();     //uncomment this to allow fish to get food after the trial ends
      gate_check();
      if(fish_got_food_yet == true) {
        RGB_off();
      }      
    ...
    Serial.print(last_cafeteria_entrance,4); Serial.print(",");
    Serial.print(fish_at_feeder_at_time,4); Serial.print(",");
    if ((first_cafeteria_exit == -1) && (last_cafeteria_entrance != -1)) {
      first_cafeteria_exit = first_cafeteria_exit_without_food;
    }
    Serial.print(first_cafeteria_exit,4); Serial.println("");
}
...
void feeder_check()
{
  ...
  //Note that the A0(feeding) sensor inteprets close objects as LOW numbers, AND the A2(gate) sensor interpets close objects as LOW numbers.
...
boolean gate_check()
{
  ...
  if (analogRead(A2) < 30) {
    if (fish_in_feeding_area == false) {
      ...
      delay(1000);
      if (fish_got_food_yet == false) {
        last_cafeteria_entrance = gate_sensor_time; }
      return true;
    }
      ...
      if (fish_got_food_yet == true) {
        if (first_cafeteria_exit == -1) {
          first_cafeteria_exit = gate_sensor_time; }}
      else {
        first_cafeteria_exit_without_food = gate_sensor_time;}
