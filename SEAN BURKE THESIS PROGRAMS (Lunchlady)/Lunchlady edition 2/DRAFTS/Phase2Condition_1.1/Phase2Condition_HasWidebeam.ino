...
boolean fish_tripped_feed_sensor = false;    //whether the fish has gotten close enough to the feeder to trigger the sensor (we assume it ate)
...
//TIMER variables
float wide_sensor_time = 0;
float feed_sensor_time = 0;
float start_time_secs = 0;
int times_passed_wide_beam = 0;
long randNumber = 0;
...
void loop()                                 
{
    times_passed_wide_beam = 0;
    fish_tripped_feed_sensor = false;                    //we'll need this later
    ...
    randNumber = random(100, 200);           //random(minimum seconds, maximum seconds)
    ...
    while ( (millis()/1000.00 - start_time_secs) < 8 ) {
      check_for_fish_WIDE();
      check_for_fish_FEED();
      if ( (fish_in_feeding_area == true) and (fish_tripped_feed_sensor == true) ) {
        dispense_food();
      }      
    }
    ...
    if (fish_tripped_feed_sensor == false) {
...
void check_for_fish_WIDE()
{
  if ( ((millis() / 1000.00) - wide_sensor_time) > 1) {               //check if it's been a second since the last time the fish tripped the sensor
    if (analogRead(A2) < 50) {                                       //check if the fish trips the beam
      Serial.println(times_passed_wide_beam);
      delay(200);
      Serial.println(analogRead(A2));
      if ( times_passed_wide_beam < 3 ) {                             //tells the program to stop recording the wide beam passing after the first 2 times
        wide_sensor_time = (millis() / 1000.00) - start_time_secs;    //record the time taken for the fish to pass the widebeam since the start of the trial
        if ( (times_passed_wide_beam % 2) == 0 ){
          Serial.print("time to pass the wide beam sensor towards the food is: "); Serial.println(wide_sensor_time, 4);
          fish_in_feeding_area == true;
        }
        else {
          Serial.print("time to pass the wide beam sensor away from the food is: "); Serial.println(wide_sensor_time, 4);
          fish_in_feeding_area == false;
        }
      }
      times_passed_wide_beam = times_passed_wide_beam + 1;
    }
  }
}


void check_for_fish_FEED()
{
  if (fish_tripped_feed_sensor == false) {             //check to make sure the "fish approached..." statement hasn't already been printed this cycle
    if (analogRead(A0) < 75) {                //do this bit if the sensor is being tripped (the 75 is essentially arbitrary):
      ...
      Serial.print("time to trip the feed sensor is: "); Serial.println(feed_sensor_time, 4);
      fish_tripped_feed_sensor = true;                                         //set this so we can tell the computer not to print the "fish approached..." statement again
    }
  }     
}
