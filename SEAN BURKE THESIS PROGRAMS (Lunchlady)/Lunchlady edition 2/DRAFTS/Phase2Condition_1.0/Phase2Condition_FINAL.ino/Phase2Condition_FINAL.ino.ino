...
void check_for_fish()
{
  ...
  if ( (analogRead(A2) > 100) && (fish_through_gate == false) ) {
    ...
    Serial.print("fish through gate in "); Serial.println(gate_sensor_time, 4);
  ...
  if (fish_got_food == false) {                 //check to make sure the "fish approached..." statement hasn't already been printed this cycle
    ...
    if (fish_through_gate == true) {
      if (analogRead(A0) < 20) {                //do this bit if the sensor is being tripped (the 75 is essentially arbitrary):
        ...
        Serial.print("fish at feeder in "); Serial.println(feed_sensor_time, 4);
  ...
  if (fish_in_feeding_area == true) {
    if (analogRead(A2) > 100) {
      if (fish_through_gate == false) {
        ...
        fish_in_feeding_area = false;
        Serial.println("fish left feeding area");
      }
    }
  }
}
